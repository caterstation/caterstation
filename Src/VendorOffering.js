import { View, Text, TouchableOpacity, FlatList, Image, Pressable, ActivityIndicator, StyleSheet, ScrollView } from 'react-native';
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            const filteredVendors = data.allvendors.filter((vendor) => {
                return vendor.vendor_services.some((service) => selectedServices.includes(service.service_name));
            });
    ...
    const handleVendorSelection = (service, vendorId) => {
        ...
        setSelectedVendors((prevSelectedVendors) => {
            ...
            if (serviceVendors.includes(vendorId)) {
                ...
                updatedAllSelectedVendorIds = allSelectedVendorIds.filter((id) => id !== vendorId);
            } else {
...
const renderServiceButton = (service) => {
        ...
        return (
            ...
            >
                <View style={{justifyContent:"center", alignItems:"center"}}>
                    <Image style={{ height: responsiveHeight(20), width: responsiveWidth(25), marginBottom: responsiveHeight(1) }} source={{ uri: imgS }} />
                <Text numberOfLines={1}  style={{ color: black, textAlign: "center", fontWeight: "bold" }}> {item.company_name}</Text>
                </View>
    ...
    const handleSubmit = async () => {
        ...
        try {
          await axios.post('https://caterstation.pro/api/custom-orders', {
            city,
        event_date,
        event_duration,
        event_location,
            ...
            <HorizontalBar backPress={() => navigation.goBack()} title="All Vendors " /> 
                ...
                <View style={styles.contentContainer}>
                    {loading ? (
                        ...
                        <View style={{ justifyContent: "center", alignItems: "center", width: responsiveWidth(100), marginBottom: responsiveHeight(5), marginTop: responsiveHeight(3) }}>
