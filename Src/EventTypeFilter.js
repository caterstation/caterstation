...
const EventTypeFilter = ({route}) => {
    const navigation = useNavigation();
    const { item } = route.params;
    const myEventType=item;
    ...
    useEffect(() => {
      const fetchData = async () => {
        ...
        const filteredPackages = data.packages.filter(
          (item) => item.event_type === myEventType.toString()
        );
      ...
      return (
        ...
        <Text style={{marginLeft:responsiveWidth(28), color:black, fontSize: responsiveFontSize(2.5), fontWeight:"bold"}}>{myEventType}</Text>
      </View>
          <FlatList
            data={packages}
                  ...
                  <View style={{ paddingHorizontal: responsiveWidth(2) }}>
                    ...
                    <View style={[styles.Row, styles.JustifyContent_spaceBetween, { marginTop: responsiveHeight(1) }]}>
                     {/* passaing data from button view detail */}
                      <Pressable
                       onPress={() => navigation.navigate("PkgEventFilter")}
                    ...
                    <View>
                      ...
                      <View style={[styles.Row, styles.JustifyContent_spaceBetween, { marginTop: 5,   marginBottom: 10,}]}>
                        ...
                        <View style={{ position: "relative" }}>
                          <TouchableOpacity onPress={()=>{navigation.navigate('PkgEventFilter',
                       {
                        item,
