...
const PackageDetails = ({ route, product }) => {
  ...
  console.log("what we get from package details : ", item,null,2)
 ...
 const catering= {
  ...
  //           <Text style={[styles.questionHeadingTxt, { color: white }]}>Frequently ask questions</Text>


  //           <Pressable style={{ marginLeft: 50, paddingHorizontal: 15, }} onPress={() => setView5('six')}>
  ...
  //           </Pressable>
  //         </View>
  //         <Text style={styles.questionTxt}>It is a long established fact that a reader will be distracted by the readable content of a page when looking at its layout. The point of using Lorem Ipsum is that it has a more-or-less normal distribution of letters.</Text>
  ...
  //         <Text style={styles.questionHeadingTxt}>Frequently ask questions</Text>
  //         <Pressable style={{ marginLeft: 50, paddingHorizontal: 15, }} onPress={() => setView5('five')}>
  ...
  //       <View style={styles.questionBox1}>
  //         <Text style={styles.questionHeadingTxt}>Frequently ask questions</Text>
  //         <Pressable style={{ marginLeft: 50, paddingHorizontal: 15, }} onPress={() => setView3('fourth')}>
  //           <FontAwsome name="angle-up" color={"black"} size={20} />
  ...
  //           <Pressable style={{ marginLeft: 50, paddingHorizontal: 15, }} onPress={() => setView3('third')}>
  //             <FontAwsome name="angle-down" color={white} size={20} />
  ...
  //           <Pressable style={{ marginLeft: 50, paddingHorizontal: 15, }} onPress={() => setView('second')}>
  //             <FontAwsome name="angle-up" color={"black"} size={20} />
  //           </Pressable>
  //         </View>
  //       );
  ...
  //             <Pressable style={{ marginLeft: 50, paddingHorizontal: 15, }} onPress={() => setView('first')}>
  //               <FontAwsome name="angle-down" color={white} size={20} />
  ...
  const modifyHtmlpackage_detail = (htmlContent) => {
  ...
  const modifiedHtmlpackage_detail = modifyHtmlpackage_detail(item.package_detail);
  
  
  
  
  
  const modifyHtmlcater= (htmlContent) => {
    // Use regular expression to add color style to <span> elements
    const modifiedHtml = htmlContent.replace(
      /<span[^>]*>(.*?)<\/span>/g,
      '<span style="color: black;">$1</span>'
  ...
  const modifiedHtmlcatering = modifyHtmlcater(item.catering); 
  
  
  
  const modifyHtmlDecore= (htmlContent) => {
    // Use regular expression to add color style to <span> elements
    const modifiedHtml = htmlContent.replace(
      /<span[^>]*>(.*?)<\/span>/g,
      '<span style="color: black;">$1</span>'
  ...
  const modifiedHtmldecore = modifyHtmlDecore(item.decor);
  
  
  const modifyHtmlVenue= (htmlContent) => {
    // Use regular expression to add color style to <span> elements
    const modifiedHtml = htmlContent.replace(
      /<span[^>]*>(.*?)<\/span>/g,
      '<span style="color: black;">$1</span>'
  ...
  const modifiedHtmlvenue= modifyHtmlVenue(item.venue);
  ...
  // <SafeAreaView style={{backgroundColor:"green"}}>
    <ScrollView style={{backgroundColor:white}}>
        ...
        <View
          ...
          <RenderHtml contentWidth={width} source={{ html: modifiedHtmlpackage_detail }} />
               
          <RenderHtml contentWidth={width} source={{ html: modifiedHtmlcatering}}  />
          <RenderHtml contentWidth={width} source={{ html: modifiedHtmldecore}}  />
          <RenderHtml contentWidth={width}source={{ html: modifiedHtmlvenue}} />
        </View>
                  ...
                  <>
                    <View>
                      <Image
                        ...
                        source={{ uri: imgS }}
                      />
                      <Text style={{ fontSize: responsiveFontSize(1.5), color: blue, textAlign: "center", marginTop: responsiveHeight(1) }}>{item.package_name}</Text>
                    </View>
                  </>
                );
              }}
